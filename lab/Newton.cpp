...
{
	// double a = 3.0*x + sin(x)-pow(2.71828,x)   // e = 2.71828
	double a = pow(x, 3.0) - 3 * x - 5.0; //write the equation whose roots are to be determined
	return a;
}
double fprime(double x);
double fprime(double x)
{
	double b =  3 * pow(x, 2) - 3; //write the first derivative of the equation
	return b;
}
int main()
{
